...
async function writeMimeList(mimesAppsList: string) {
  if (!existsSync(mimesAppsList)) {
    await writeFile(mimesAppsList, ['[Default Applications]', 'x-scheme-handler/nitindyt-launcher-plus=nitindyt-launcher-plus.desktop;\n'].join('\n'))
    return
  }
  const content = await readFile(mimesAppsList, 'utf-8')
  if (content.indexOf('x-scheme-handler/nitindyt-launcher-plus=nitindyt-launcher-plus.desktop') === -1) {
    ...
    if (defaultAppsHeaderIndex === -1) {
      ...
      lines.push('x-scheme-handler/nitindyt-launcher-plus=nitindyt-launcher-plus.desktop;')
    } else {
      lines = [...lines.slice(0, defaultAppsHeaderIndex + 1), 'x-scheme-handler/nitindyt-launcher-plus=nitindyt-launcher-plus.desktop;', ...lines.slice(defaultAppsHeaderIndex + 1)]
    }
...
async function ensureDesktopFile(homePath: string, exePath: string, assigned: boolean) {
  const desktopFile = join(homePath, '.local', 'share', 'applications', 'nitindyt-launcher-plus.desktop')
  if (existsSync(desktopFile) || !assigned) {
    await writeFile(desktopFile, `[Desktop Entry]\nName=NITINDYT LAUNCHER+\nExec=${exePath} %u\nIcon=${exePath}\nType=Application\nMimeType=x-scheme-handler/nitindyt-launcher-plus;`)
  }
}

export async function setLinuxProtocol(homePath: string, exePath: string) {
  const existed = execSync('xdg-settings get default-url-scheme-handler nitindyt-launcher-plus', { encoding: 'utf-8' }).trim()
